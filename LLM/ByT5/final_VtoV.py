...
# Load datasets
TRAIN_PATH = "Training Data Public Upload/train_data_10k_wv2v.json"
EVAL_PATH = "Training Data Public Upload/eval_data_1k_wv2v.json"
TEST_PATH = "Training Data Public Upload/test_data_1k_wv2v.json"

train_df = pd.read_json(TRAIN_PATH).rename(columns={"PFD": "text", "PID": "label"})
eval_df = pd.read_json(EVAL_PATH).rename(columns={"PFD": "text", "PID": "label"})
...
def tokenize_fn(batch):
    inputs = tokenizer(batch["text"], max_length=max_length, padding="longest")
    labels = tokenizer(batch["label"], max_length=max_length, padding="longest")
...
training_args = Seq2SeqTrainingArguments(
    output_dir="./byt5-finetuned-model-v2v-run1",
    eval_strategy="steps",
    save_strategy="steps",
    report_to="wandb",
    run_name="byt5-finetuned-model-v2v-run1",
    eval_steps=500,
    save_steps=500,
    logging_steps=500,
    num_train_epochs=10,
...
# Save model
model.save_pretrained("./byt5-finetuned-model-v2v-run1")
tokenizer.save_pretrained("./byt5-finetuned-model-v2v-run1")
