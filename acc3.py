...
for i in tqdm(range(0, len(batch_inputs), batch_size), desc="Evaluating"):
    ...
    for j in range(len(batch)):
        ...
        if matches == 0:
            ...
            most_common_pred, freq = pred_counter.most_common(1)[0]

            if freq >= 2:
                print(f"\n Consistent Wrong Prediction Detected in Sample {total_samples}:")
              
                print(f"Repeated Prediction: '{most_common_pred}' appeared {freq} times")
...
print(f"\nTotal Confident Wrong Predictions: {len(confident_wrong_cases)}")

with open("confident_wrong_predictions1.json", "w") as f:
    json.dump(confident_wrong_cases, f, indent=2)