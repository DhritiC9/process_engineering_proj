...
def plot_flowsheet_nx(graph, plot_with_stream_labels, add_positions=True):
    ...
    fig = plt.figure(figsize=(flowsheet_size * 18. / 6., flowsheet_size * 12. / 6.))
    ...
    if plot_with_stream_labels:
        nx.draw(graph, pos, with_labels=True, node_size=5., font_size=13, node_color="#0033d9")
        try:
            labels = dict([((n1, n2),
        ...
        node_initial_size = 20000
        node_size_multiple = 700
        node_size =  node_initial_size + node_size_multiple * n

        font_initial_size = 18
        font_size_div = 2
        font_size =  font_initial_size + n // font_size_div

        arrow_initial_size = 40
        arrow_size_multiple = 1.3
        arrowsize = arrow_initial_size + arrow_size_multiple*n
        #linewidths = 2 + n*2
        ...
        # controllers: pressure/flow/level indicators
            if any(ctrl in n for ctrl in ["PC", "FC", "LC","TC","M","FRC","FFC"]):
                color_map.append("#1f78b4")  
                controllers+=1
            # inidicators:
            elif any(ctrl in n for ctrl in ["TI","PI","FI"] ):
                color_map.append("#33a02c")  
                indicators+=1
            elif any(ctrl in n for ctrl in ["LT","FT"]):
                color_map.append("#86cfff")
                transmitters+=1
            # everything else:
            else:
                color_map.append("#a9dd7c")
        ...
        unit_operations = flowsheet_size - controllers - indicators - transmitters
        print(f"Number of unit operations = {unit_operations}")
        print(f"Number of controllers = {controllers}")
        print(f"Number of indicators = {indicators} ")
        print(f"Number of transmitters = {transmitters}")
    plt.show()
    return fig
